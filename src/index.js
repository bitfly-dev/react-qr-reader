...
module.exports = class Reader extends Component {
  ...
  initiate(props = this.props) {
    ...
    const getUserMedia = (count = -1) => {
      const vConstraintsPromise =
        count === -1 && (supported.facingMode || isFirefox)
          ? Promise.resolve(props.constraints || constraints)
          : getDeviceId(facingMode, count).then((deviceId) =>
              Object.assign({}, { deviceId }, props.constraints)
            );

      return vConstraintsPromise
    ...
    getUserMedia().then(this.handleVideo).catch(onError);
  }
  handleVideo(stream) {
